...
final class BatchParser implements ParserInterface
{
    ...
    public function parse(Request $request): array
    {
        ...
        foreach ($queries as $i => &$query) {
            ...
            if (!is_string($query[self::PARAM_QUERY])) {
                throw new BadRequestHttpException(sprintf('%s is not a valid query', json_encode($query[self::PARAM_QUERY])));
            }
        }
